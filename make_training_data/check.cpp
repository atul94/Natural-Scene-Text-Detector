...
double find_in_GT(Rect A,vector <Rect> &GT_bounding_box)
{
	...
	for(int i = 0; i < GT_bounding_box.size(); i++)
	{
		...
		if(area_intersect/total_area >= 0.50)
			return 1.0;
...
///////////////////////////////////
void calculate_properties(vector <vector < Point > > &r,vector <region_properties> &r2, vector<vector<description> > &KeyPoints, Mat &bwimg, int mul1, int mul2, vector <Rect> &GT_bounding_box, Mat &color_img)
{
    ...
    { 
    	...
    	t.CSA = stroke_width_filter(color_img,rrr);
    	if(t.CSA < 0)
    		continue;
    	region_property[3] = t.CSA;
		region_property[4] = ((double)t.bounding_box.height)/((double)t.bounding_box.width);
        ...
        t.property = find_in_GT(t.bounding_box,GT_bounding_box);
     
        	r2.push_back(t);
        myfile << setprecision(6);
        myfile << region_property[0];
        for(int j = 1; j < 6; j++)
        	myfile << " " << region_property[j];
        myfile << " " << t.property << "\n";
...
void Create_Bounding_Box(vector <Rect> &GT_bounding_box, string ground_truth_path)
{
	ifstream infile(ground_truth_path.c_str());
	//cout << ground_truth_path << "\n";
	...
	while(getline(infile,line))
	{
		...
		x1 = xx[0]; y1 = xx[1]; x2 = xx[2]; y2 = xx[3];
		Rect r;
		r.x = x1;
		r.y = y1;
		r.width = abs(x2-x1);
		r.height = abs(y2-y1);
		GT_bounding_box.push_back(r);
	}
}
...
int main(int argc, char const *argv[])
{
    ...
    string ground_truth_path = image_path.substr(0, image_path.size()-7) + "segmentations/" + image_path.substr(image_path.size()-7,3) + "_GT.txt";
	vector <Rect> GT_bounding_box;
	Create_Bounding_Box(GT_bounding_box, ground_truth_path);
	//train_();
    ...
    string OUTPUT_FILE; 
    OUTPUT_FILE = "./o1/" + INP_STR.substr(INP_STR.size() - 7, 3) + ".txt";
	...
	for(int ii = start; ii < end; ii++)
	{
		...
		calculate_properties(regions2,property_vector, KeyPoints, bwimg,mul1,mul2,GT_bounding_box,img);
	}
	/*Mat img4;
	im.copyTo(img4);
	...
	{
		if(property_vector[i].property == -1.0)
			continue;
